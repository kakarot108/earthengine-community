...
// A Landsat 8 TOA image collection (3 months of images at a specific point).
var col = ee.ImageCollection('LANDSAT/LC08/C02/T1_TOA')
  .filterBounds(ee.Geometry.Point(-90.70, 34.71))
  .filterDate('2020-07-01', '2020-10-01');
print('Collection', col);

// For this basic demonstration, limit the collection size to 4 images and
// use only bands B2, B3, and B4.
var colLimit = col.limit(4).select('B[2-4]');
...
Map.addLayer(colLimit.first(), visParams, 'Image representative');
...
var pixelInfoBbox = colLimit.getRegion({
  geometry: roi,
  scale: 30
});
...
// that there are 4 images and the region intersects 2 pixels, so n rows
// equals 9 (4 * 2 + 1). All collection images must have the same number of
...
var pixelInfoPoints = colLimit.getRegion({
  geometry: points,
  scale: 30
});
